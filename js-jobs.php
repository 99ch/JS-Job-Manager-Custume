 ...
 * @version 2.0.2 - Customed version
 */
/*
  ...
  Author: JoomSky (Customed by keoni-consulting.net)
  Version: 2.0.2 - Customed version
  Text Domain: js-jobs
...
class jsjobs {
    ...
    function js_for_google_map() {
        ...
        $autoload_query = rawurlencode(wp_json_encode($autoload));
        $chart_url = 'https://www.google.com/jsapi?autoload=' . $autoload_query;
        wp_enqueue_script('jsjobs-googlaChart', $chart_url);
    }
...
function jsjobs_login_redirect($redirect_to, $request, $user){
   ...
   if ( isset( $user->roles ) && is_array( $user->roles ) ) {
       ...
       if ( in_array( 'administrator', $user->roles ) ) {
           ...
           $query = "SELECT roleid FROM `".jsjobs::$_db->prefix."js_job_users` WHERE uid = " . esc_sql($user->id);
