            ...
            switch ($layout) {
                ...
                case 'admin_viewresume':
                    ...
                    if (JSJOBSincluder::getObjectClass('user')->isjobseeker() || current_user_can('manage_options') || (JSJOBSincluder::getObjectClass('user')->isemployer() && $empflag == 1) || JSJOBSincluder::getJSModel('configuration')->getConfigurationByConfigName('visitorview_emp_viewresume') == 1 ) {
                        ...
                        jsjobs::$_data['debug_resume'] = array(
                        ...
                        $forceView = (int) JSJOBSrequest::getVar('forceview', null, 0) === 1;
                        jsjobs::$_data['debug_resume']['forceView'] = $forceView;
                        ...
                        if ((!is_numeric($resumeid) && $resumeid[0] . $resumeid[1] . $resumeid[2] == 'sc-') || $socialid != null) { // social
                            ...
                            jsjobs::$_data['debug_resume']['parsedId'] = $resumeid;
                            ...
                            if (JSJOBSincluder::getObjectClass('user')->isjobseeker()) {
                                if (JSJOBSincluder::getJSModel('resume')->getIfResumeOwner($resumeid)) {
                                    ...
                                    if ($forceView) {
                                        jsjobs::$_data['debug_resume']['access'] = 'owner_forced';
                                        $expiryflag = true;
                                    } else {
                                        jsjobs::$_error_flag_message = JSJOBSLayout::setMessageFor(10,null,null,1);
                                        jsjobs::$_error_flag_message_for = 2;
                                        jsjobs::$_error_flag = true;
                                        jsjobs::$_data['debug_resume']['access'] = 'owner_mismatch';
                                        $canLoadResume = false;
                                    }
                                }
                            }
                            if ($canLoadResume && $expiryflag == false) {
                                if ($forceView) {
                                    jsjobs::$_data['debug_resume']['access'] = 'expired_forced';
                                    $expiryflag = true;
                                } else {
                                    jsjobs::$_error_flag_message = JSJOBSLayout::setMessageFor(6,null,null,1);
                                    jsjobs::$_error_flag_message_for=6;
                                    jsjobs::$_error_flag = true;
                                    jsjobs::$_data['debug_resume']['access'] = 'expired_blocked';
                                    $canLoadResume = false;
                                }
                            }
                            if ($canLoadResume) {
                                ...
                                jsjobs::$_data['debug_resume']['access'] = $forceView ? 'loaded_forced' : 'loaded';
                                jsjobs::$_data['debug_resume']['resumeStatus'] = isset(jsjobs::$_data[0]['personal_section']->status) ? jsjobs::$_data[0]['personal_section']->status : null;
                            } else {
                                break;
                            }
                        }
                ...
                case 'resumebycategory':
                    if (is_admin() || (JSJOBSincluder::getObjectClass('user')->isemployer() && $empflag == 1) || JSJOBSincluder::getJSModel('configuration')->getConfigurationByConfigName('visitorview_emp_resumecat') == 1 ) {
                        ...
                        jsjobs::$_error_flag = true;
                        jsjobs::$_data['debug_resume']['access'] = 'blocked_role';
                    }
                    break;
                ...
                case 'addresume':
                    ...
                    $guestallowed = 0;
                    $jobseekerFormEnabled = isset($config_array['formresume']) ? ((int)$config_array['formresume'] === 1) : true;
                    $visitorFormEnabled = isset($config_array['vis_jsformresume']) ? ((int)$config_array['vis_jsformresume'] === 1) : false;

                    if ($guest) {
                        $guestallowed = isset($config_array['visitor_can_add_resume']) ? (int)$config_array['visitor_can_add_resume'] : 0;
                    }
                    if ((JSJOBSincluder::getObjectClass('user')->isjobseeker() && $jobseekerFormEnabled) || ($guestallowed == 1 && $visitorFormEnabled) || is_admin()) {
