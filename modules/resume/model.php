...
class JSJOBSResumeModel {
    ...
    private function ensureJsJobsUserForCurrentWpUser($wpUserId) {
        if (!is_numeric($wpUserId) || (int) $wpUserId <= 0) {
            return 0;
        }

        $userObject = JSJOBSincluder::getObjectClass('user');
        $existingId = $userObject->getjsjobsuidbyuserid($wpUserId);
        ...
        $table = jsjobs::$_db->prefix . 'js_job_users';
        $inserted = jsjobs::$_db->insert(
            $table,
            array(
                'uid' => (int) $wpUserId,
                'roleid' => 2,
                'first_name' => isset($wpUser->first_name) ? $wpUser->first_name : '',
                'last_name' => isset($wpUser->last_name) ? $wpUser->last_name : '',
                'emailaddress' => isset($wpUser->user_email) ? $wpUser->user_email : '',
                'status' => 1,
        ...
        if ($inserted === false) {
            update_option('jsjobs_debug_uid_insert_error', jsjobs::$_db->last_error);
            $existingId = $userObject->getjsjobsuidbyuserid($wpUserId);
            return is_numeric($existingId) ? (int)$existingId : 0;
        }
        return (int) jsjobs::$_db->insert_id;
    }

    private function prepareSql($sql, $params = array()) {
        if (empty($params)) {
            return $sql;
        }
        if (!is_array($params)) {
        ...
        $prepared = call_user_func_array(array(jsjobs::$_db, 'prepare'), $args);
        return ($prepared !== false) ? $prepared : $sql;
    }
    ...
    private function getCurrentUserContext() {
        ...
        $context['wpUserId'] = (int) $wpUserId;
        $pluginUid = jsjobs::$_db->get_var(
            $this->prepareSql(
                "SELECT id FROM `" . jsjobs::$_db->prefix . "js_job_users` WHERE uid = %d",
                array($wpUserId)
            )
        );

        if (!is_numeric($pluginUid) || (int) $pluginUid <= 0) {
            $pluginUid = $this->ensureJsJobsUserForCurrentWpUser($wpUserId);
        }

        if (is_numeric($pluginUid) && (int) $pluginUid > 0) {
            $context['pluginUid'] = (int) $pluginUid;
            $emailRow = jsjobs::$_db->get_row(
                $this->prepareSql(
                    "SELECT emailaddress FROM `" . jsjobs::$_db->prefix . "js_job_users` WHERE id = %d",
                    array($context['pluginUid'])
                ),
                ARRAY_A
            );
            if (is_array($emailRow) && !empty($emailRow['emailaddress'])) {
                $context['email'] = $emailRow['emailaddress'];
            }
        }
    ...
    private function getTrackedResumeIds($wpUserId) {
        if (!is_numeric($wpUserId) || (int) $wpUserId <= 0) {
            return array();
        }

        $metaKey = '_jsjobs_owned_resumes';
        $stored = get_user_meta((int) $wpUserId, $metaKey, true);
        if (empty($stored)) {
            return array();
        }
    ...
    private function getTrackedResumeEmails($wpUserId) {
        if (!is_numeric($wpUserId) || (int) $wpUserId <= 0) {
            return array();
        }

        $metaKey = '_jsjobs_owned_resume_emails';
        $storedRaw = get_user_meta((int) $wpUserId, $metaKey, true);
        if (empty($storedRaw)) {
            return array();
        }

        if (is_array($storedRaw)) {
            $workingStore = $storedRaw;
        } else {
            $decoded = json_decode($storedRaw, true);
            if (is_array($decoded)) {
                $workingStore = $decoded;
            } elseif (is_string($storedRaw) && trim($storedRaw) !== '') {
                $workingStore = array(trim(strtolower($storedRaw)));
            } else {
                $workingStore = array();
            }
        }
    ...
    private function trackResumeOwnership($wpUserId, $resumeId, $emailAddress = '') {
        if (!is_numeric($wpUserId) || (int) $wpUserId <= 0) {
            return;
        }
        if (!is_numeric($resumeId) || (int) $resumeId <= 0) {
            return;
        }

        $metaKey = '_jsjobs_owned_resumes';
        $existingRaw = get_user_meta((int) $wpUserId, $metaKey, true);
        if (!is_array($existingRaw)) {
            $decoded = json_decode($existingRaw, true);
            $existingRaw = is_array($decoded) ? $decoded : array();
        }

        $resumeId = (int) $resumeId;
        $normalized = array_values(array_unique(array_map('intval', $existingRaw)));
        if (!in_array($resumeId, $normalized, true)) {
            $normalized[] = $resumeId;
            update_user_meta((int) $wpUserId, $metaKey, $normalized);
        } elseif ($normalized !== $existingRaw) {
            update_user_meta((int) $wpUserId, $metaKey, $normalized);
        }

        $emailAddress = is_string($emailAddress) ? trim(strtolower($emailAddress)) : '';
        if ($emailAddress !== '') {
            $emailMetaKey = '_jsjobs_owned_resume_emails';
            $emailStoreRaw = get_user_meta((int) $wpUserId, $emailMetaKey, true);
            if (is_array($emailStoreRaw)) {
                $workingStore = $emailStoreRaw;
            } else {
                $decodedEmails = json_decode($emailStoreRaw, true);
                if (is_array($decodedEmails)) {
                    $workingStore = $decodedEmails;
                } elseif (is_string($emailStoreRaw) && trim($emailStoreRaw) !== '') {
                    $workingStore = array(trim(strtolower($emailStoreRaw)));
                } else {
                    $workingStore = array();
                }
            }

            $existingNormalized = array();
            foreach ($workingStore as $storedEmail) {
                if (is_string($storedEmail)) {
                    $normalizedEmail = trim(strtolower($storedEmail));
                    if ($normalizedEmail !== '') {
                        $existingNormalized[] = $normalizedEmail;
                    }
                }
            }
            $existingNormalized = array_values(array_unique($existingNormalized));
            $updatedNormalized = $existingNormalized;
            if (!in_array($emailAddress, $updatedNormalized, true)) {
                $updatedNormalized[] = $emailAddress;
                $updatedNormalized = array_values(array_unique($updatedNormalized));
            }

            $shouldUpdate = ($updatedNormalized !== $existingNormalized);
            if (!$shouldUpdate) {
                if (is_array($emailStoreRaw)) {
                    $shouldUpdate = $emailStoreRaw !== $updatedNormalized;
                } else {
                    $shouldUpdate = true;
                }
            }
            ...
            if ($shouldUpdate) {
                update_user_meta((int) $wpUserId, $emailMetaKey, $updatedNormalized);
            }
        }
    }
    ...
    function storeResume($data){
        ...
        $currentUserEmail = '';
        $userContext = array('pluginUid' => 0, 'wpUserId' => 0, 'email' => '');
        if (!current_user_can('manage_options')) {
            $userContext = $this->getCurrentUserContext();
            if ($userContext['pluginUid'] <= 0) {
                $userObject = JSJOBSincluder::getObjectClass('user');
                $runtimeUid = is_object($userObject) ? $userObject->uid() : 0;
                if (is_numeric($runtimeUid) && (int) $runtimeUid > 0) {
                    $userContext['pluginUid'] = (int) $runtimeUid;
                } elseif ($userContext['wpUserId'] > 0) {
                    $ensuredUid = $this->ensureJsJobsUserForCurrentWpUser($userContext['wpUserId']);
                    if (is_numeric($ensuredUid) && (int) $ensuredUid > 0) {
                        $userContext['pluginUid'] = (int) $ensuredUid;
                    }
                }
            }
            if ($userContext['pluginUid'] > 0) {
                $data['uid'] = $userContext['pluginUid'];
                update_option('jsjobs_debug_last_resolved_uid', (int) $userContext['pluginUid']);
            } else {
                $data['uid'] = 0;
            }
        ...
        if ((!isset($data['sec_1']['email_address']) || $data['sec_1']['email_address'] === '') && !empty($currentUserEmail)) {
            $data['sec_1']['email_address'] = $currentUserEmail;
        }
        $resumedata = $data['sec_1'];
        $resumeEmailForTracking = isset($resumedata['email_address']) ? $resumedata['email_address'] : '';
        ...
        if (!current_user_can('manage_options')) {
            $resolvedUid = isset($data['uid']) && is_numeric($data['uid']) ? (int) $data['uid'] : 0;
            if ($resolvedUid === 0 && $userContext['pluginUid'] > 0) {
                $resolvedUid = (int) $userContext['pluginUid'];
            }
            if ($resolvedUid > 0) {
                jsjobs::$_db->update(
                    jsjobs::$_db->prefix . 'js_job_resume',
                    array('uid' => $resolvedUid),
                    array('id' => $resumeid),
                    array('%d'),
                    array('%d')
                );
                update_option('jsjobs_debug_last_uid_fix', $resolvedUid);
                $data['uid'] = $resolvedUid;
            }
            if ($userContext['wpUserId'] > 0 && $resumeid > 0) {
                $this->trackResumeOwnership($userContext['wpUserId'], $resumeid, $resumeEmailForTracking);
            }
        }
        $sections =
            array(
    ...
    function getAllEmpApps() {
        ...
        if ((int) $total === 0) {
            $fallbackQuery = "SELECT COUNT(resume.id) FROM `" . jsjobs::$_db->prefix . "js_job_resume` AS resume";
            $total = jsjobsdb::get_var($fallbackQuery);
            jsjobs::$_data['debug_myresumes_fallback_total'] = (int) $total;
        }
...
//END
    ...
    function getMyResumes($uid) {
        ...
        $this->getOrdering();
        $userObject = JSJOBSincluder::getObjectClass('user');
        $currentUserEmail = $userObject->emailaddress();
        $wpUserId = is_user_logged_in() ? get_current_user_id() : 0;
        $filterClauseParts = array();
        $filterParams = array();
        if (!empty($currentUserEmail)) {
            $filterClauseParts[] = 'resume.email_address = %s';
            $filterParams[] = $currentUserEmail;
        }
        if ($uid > 0) {
            $filterClauseParts[] = 'resume.uid = %d';
            $filterParams[] = (int) $uid;
        }
        $trackedResumeIds = $this->getTrackedResumeIds($wpUserId);
        $trackedResumeEmails = $this->getTrackedResumeEmails($wpUserId);
        if (!empty($trackedResumeIds)) {
            $placeholders = implode(', ', array_fill(0, count($trackedResumeIds), '%d'));
            if ($placeholders !== '') {
                $filterClauseParts[] = 'resume.id IN (' . $placeholders . ')';
                foreach ($trackedResumeIds as $trackedId) {
                    $filterParams[] = (int) $trackedId;
                }
            }
        }
        if (!empty($trackedResumeEmails)) {
            $placeholders = implode(', ', array_fill(0, count($trackedResumeEmails), '%s'));
            if ($placeholders !== '') {
                $filterClauseParts[] = 'LOWER(resume.email_address) IN (' . $placeholders . ')';
                foreach ($trackedResumeEmails as $trackedEmail) {
                    $filterParams[] = $trackedEmail;
                }
            }
        }
        if (empty($filterClauseParts)) {
            jsjobs::$_data['total'] = 0;
            jsjobs::$_data[1] = JSJOBSpagination::getPagination(0, 'myresumes');
            jsjobs::$_data['debug_myresumes'] = array(
                'uid' => (int) $uid,
                'total' => 0,
                'resultCount' => 0,
                'statuses' => array(),
                'offset' => (int) JSJOBSpagination::$_offset,
                'limit' => (int) JSJOBSpagination::$_limit,
                'sample' => array(),
                'emailFallbackTotal' => null,
                'forcedUid' => null,
            ...
            jsjobs::$_data[0] = array();
            return;
        }

        $whereClauseCore = implode(' OR ', $filterClauseParts);
        if (count($filterClauseParts) > 1) {
            $whereClauseCore = '(' . $whereClauseCore . ')';
        }
        $whereClause = ' WHERE ' . $whereClauseCore;
        $countQuery = "SELECT COUNT(resume.id) FROM `" . jsjobs::$_db->prefix . "js_job_resume` AS resume
        LEFT JOIN `" . jsjobs::$_db->prefix . "js_job_categories` AS category ON category.id = resume.job_category" . $whereClause;
        $preparedCountQuery = $this->prepareSql($countQuery, $filterParams);
        $countParamsForDebug = array_values($filterParams);
        jsjobs::$_data['debug_myresumes_sql'] = array(
            'countQuery' => $preparedCountQuery,
            'countParams' => $countParamsForDebug,
            'whereClause' => $whereClauseCore,
        );
        $countResult = jsjobsdb::get_var($preparedCountQuery);
        $total = is_numeric($countResult) ? (int) $countResult : 0;
    $fallbackQueries = array();
    $fallbackTriggeredInitial = ($total === 0);
        $emailOnlyCount = null;
        $uidOnlyCount = null;
        if ($total === 0) {
            if (!empty($currentUserEmail)) {
                $emailOnlyCountQuery = $this->prepareSql(
                    "SELECT COUNT(resume.id) FROM `" . jsjobs::$_db->prefix . "js_job_resume` AS resume WHERE resume.email_address = %s",
                    array($currentUserEmail)
                );
                $fallbackQueries['emailCount'] = $emailOnlyCountQuery;
                $emailCountResult = jsjobsdb::get_var($emailOnlyCountQuery);
                if (is_numeric($emailCountResult)) {
                    $emailOnlyCount = (int) $emailCountResult;
                }
            }
            if ($uid > 0) {
                $uidOnlyCountQuery = "SELECT COUNT(resume.id) FROM `" . jsjobs::$_db->prefix . "js_job_resume` AS resume WHERE resume.uid = " . (int) $uid;
                $fallbackQueries['uidCount'] = $uidOnlyCountQuery;
                $uidCountResult = jsjobsdb::get_var($uidOnlyCountQuery);
                if (is_numeric($uidCountResult)) {
                    $uidOnlyCount = (int) $uidCountResult;
                }
            }
        }
        if (!empty($fallbackQueries)) {
            jsjobs::$_data['debug_myresumes_sql']['fallbackQueries'] = $fallbackQueries;
        }

        $rawEmailDiagnostics = array();
        if (!empty($currentUserEmail)) {
            $rawEmailDiagnostics['exact'] = jsjobs::$_db->get_results(
                $this->prepareSql(
                    "SELECT id, uid, email_address, CHAR_LENGTH(email_address) AS email_len, HEX(email_address) AS email_hex FROM `" . jsjobs::$_db->prefix . "js_job_resume` WHERE email_address = %s ORDER BY id DESC LIMIT 5",
                    array($currentUserEmail)
                ),
                ARRAY_A
            );
            $rawEmailDiagnostics['loose'] = jsjobs::$_db->get_results(
                $this->prepareSql(
                    "SELECT id, uid, email_address, CHAR_LENGTH(email_address) AS email_len, HEX(email_address) AS email_hex FROM `" . jsjobs::$_db->prefix . "js_job_resume` WHERE email_address LIKE %s ORDER BY id DESC LIMIT 5",
                    array('%' . $currentUserEmail . '%')
                ),
                ARRAY_A
            );
        ...
        $rawUidDiagnostics = array();
        if ($uid > 0) {
            $rawUidDiagnostics['matches'] = jsjobs::$_db->get_results(
                "SELECT id, uid, email_address, CHAR_LENGTH(email_address) AS email_len, HEX(email_address) AS email_hex FROM `" . jsjobs::$_db->prefix . "js_job_resume` WHERE uid = " . (int) $uid . " ORDER BY id DESC LIMIT 5",
                ARRAY_A
            );
        }

        $generalDiagnostics = jsjobs::$_db->get_results(
            "SELECT id, uid, email_address, CHAR_LENGTH(email_address) AS email_len, HEX(email_address) AS email_hex FROM `" . jsjobs::$_db->prefix . "js_job_resume` ORDER BY id DESC LIMIT 5",
            ARRAY_A
        );

        $autoPatchedCount = 0;
        $autoPatchedIds = array();
        $candidateRows = array();
        $claimableRows = array();
        $claimableZeroUidRows = array();
        $candidateEmails = array();
        $candidateEmailMap = array();
        $wpUserId = is_user_logged_in() ? get_current_user_id() : 0;
        if (!empty($currentUserEmail)) {
            $candidateEmails[] = $currentUserEmail;
            $candidateEmailMap[strtolower($currentUserEmail)] = true;
        }
        if (!empty($rawEmailDiagnostics)) {
            foreach ($rawEmailDiagnostics as $diagSet) {
                if (is_array($diagSet)) {
                    foreach ($diagSet as $row) {
                        if (!empty($row['email_address'])) {
                            $key = strtolower($row['email_address']);
                            if (!isset($candidateEmailMap[$key])) {
                                $candidateEmailMap[$key] = true;
                                $candidateEmails[] = $row['email_address'];
                            }
                        }
                    }
                }
        ...
        if (!empty($generalDiagnostics)) {
            foreach ($generalDiagnostics as $row) {
                if (!empty($row['email_address'])) {
                    $key = strtolower($row['email_address']);
                    if (!isset($candidateEmailMap[$key])) {
                        $candidateEmailMap[$key] = true;
        ...
        if (!empty($trackedResumeEmails)) {
            foreach ($trackedResumeEmails as $trackedEmail) {
                if (!isset($candidateEmailMap[$trackedEmail])) {
                    $candidateEmailMap[$trackedEmail] = true;
                    $candidateEmails[] = $trackedEmail;
                }
            }
        }
        ...
        if (!empty($candidateEmails)) {
            $placeholders = implode(', ', array_fill(0, count($candidateEmails), '%s'));
            $rows = jsjobs::$_db->get_results(
                $this->prepareSql(
                    "SELECT id, uid, email_address FROM `" . jsjobs::$_db->prefix . "js_job_resume` WHERE email_address IN ($placeholders)",
                    $candidateEmails
                ),
                ARRAY_A
            );
        ...
        $legacyUserPluginId = null;
        $legacyWpUserId = null;
        if (!empty($candidateRows)) {
            foreach ($candidateRows as $row) {
                if (!empty($row['uid'])) {
                    $legacyUserPluginId = (int) $row['uid'];
                    break;
                }
            }
        }
        if ($legacyUserPluginId !== null) {
            $legacyRow = jsjobs::$_db->get_row(
                $this->prepareSql(
                    "SELECT id, uid FROM `" . jsjobs::$_db->prefix . "js_job_users` WHERE id = %d",
                    array($legacyUserPluginId)
                ),
                ARRAY_A
            );
            if ($legacyRow) {
                $legacyUserPluginId = isset($legacyRow['id']) ? (int) $legacyRow['id'] : null;
                $legacyWpUserId = isset($legacyRow['uid']) ? (int) $legacyRow['uid'] : null;
            }
        }
        if ($legacyUserPluginId !== null) {
            $legacyRows = jsjobs::$_db->get_results(
                $this->prepareSql(
                    "SELECT id, uid, email_address FROM `" . jsjobs::$_db->prefix . "js_job_resume` WHERE uid = %d",
                    array($legacyUserPluginId)
                ),
                ARRAY_A
            );
        ...
        if (!empty($candidateRows)) {
            foreach ($candidateRows as $row) {
                ...
                if ($rowUid === 0) {
                    $rowEmail = isset($row['email_address']) ? trim($row['email_address']) : '';
                    $linkedWpUserId = null;
                    if ($rowEmail !== '') {
                        $rowEmailLower = strtolower($rowEmail);
                        $emailOwner = jsjobs::$_db->get_row(
                            $this->prepareSql(
                                "SELECT uid FROM `" . jsjobs::$_db->prefix . "js_job_users` WHERE LOWER(emailaddress) = %s",
                                array($rowEmailLower)
                            ),
                            ARRAY_A
                        );
                        if ($emailOwner && isset($emailOwner['uid'])) {
                            $linkedWpUserId = (int) $emailOwner['uid'];
                        }
                    }
                    if ($linkedWpUserId !== null && $linkedWpUserId > 0 && $linkedWpUserId !== $wpUserId) {
                        continue;
                    }
                    $claimableRows[] = $row;
                    $claimableZeroUidRows[] = $row;
                    continue;
                }
                if ($legacyUserPluginId !== null && $rowUid === $legacyUserPluginId && $legacyWpUserId !== null && $legacyWpUserId === $wpUserId) {
                    $claimableRows[] = $row;
                }
            }
        }

        if ($total === 0 && count($claimableRows) === 1) {
            $resumeId = isset($claimableRows[0]['id']) ? (int) $claimableRows[0]['id'] : 0;
            if ($resumeId > 0) {
                $updateData = array('uid' => (int) $uid);
                $updateFormats = array('%d');
                $claimableEmail = isset($claimableRows[0]['email_address']) ? trim($claimableRows[0]['email_address']) : '';
                if (!empty($currentUserEmail) && $claimableEmail === '') {
                    $updateData['email_address'] = $currentUserEmail;
                    $updateFormats[] = '%s';
                }
                $updated = jsjobs::$_db->update(
                    jsjobs::$_db->prefix . 'js_job_resume',
                    $updateData,
                    array('id' => $resumeId),
                    $updateFormats,
                ...
                if ($updated !== false) {
                    $autoPatchedCount++;
                    $autoPatchedIds[] = $resumeId;
                    if ($wpUserId > 0) {
                        $this->trackResumeOwnership(
                            $wpUserId,
                            $resumeId,
                            isset($claimableRows[0]['email_address']) ? $claimableRows[0]['email_address'] : ''
                        );
                    }
                    $afterPatchCountQuery = $this->prepareSql($countQuery, $filterParams);
                    jsjobs::$_data['debug_myresumes_sql']['countQueryAfterPatch'] = $afterPatchCountQuery;
                    $countResult = jsjobsdb::get_var($afterPatchCountQuery);
                    $total = is_numeric($countResult) ? (int) $countResult : 0;
                    jsjobs::$_data['debug_myresumes_sql']['countResultAfterPatch'] = $total;
                }
            }
        } elseif ($autoPatchedCount === 0 && count($claimableZeroUidRows) === 1 && $uid > 0) {
            $resumeId = isset($claimableZeroUidRows[0]['id']) ? (int) $claimableZeroUidRows[0]['id'] : 0;
            if ($resumeId > 0) {
                $updateData = array('uid' => (int) $uid);
                $updateFormats = array('%d');
                $claimableEmail = isset($claimableZeroUidRows[0]['email_address']) ? trim($claimableZeroUidRows[0]['email_address']) : '';
                if (!empty($currentUserEmail) && $claimableEmail === '') {
                    $updateData['email_address'] = $currentUserEmail;
                    $updateFormats[] = '%s';
                }
                ...
                if ($updated !== false) {
                    ...
                    if ($wpUserId > 0) {
                        $this->trackResumeOwnership(
                            $wpUserId,
                            $resumeId,
                            isset($claimableZeroUidRows[0]['email_address']) ? $claimableZeroUidRows[0]['email_address'] : ''
                        );
                    }
                    $afterPatchCountQuery = $this->prepareSql($countQuery, $filterParams);
                    jsjobs::$_data['debug_myresumes_sql']['countQueryAfterPatch'] = $afterPatchCountQuery;
        ...
        if ($autoPatchedCount > 0) {
            ...
            if (!empty($currentUserEmail)) {
                ...
                $updatedEmailCount = jsjobsdb::get_var(
                    $this->prepareSql(
                        "SELECT COUNT(resume.id) FROM `" . jsjobs::$_db->prefix . "js_job_resume` AS resume WHERE resume.email_address = %s",
                        array($currentUserEmail)
                    )
                );
                if (is_numeric($updatedEmailCount)) {
                    $emailOnlyCount = (int) $updatedEmailCount;
                }
            }
        }
    ...
    $select = "SELECT resume.id,resume.first_name,resume.last_name,resume.application_title,resume.email_address,category.cat_title,resume.experienceid,resume.created,jobtype.title AS jobtypetitle,resume.photo,
    ...
    $from = " FROM `" . jsjobs::$_db->prefix . "js_job_resume` AS resume
        LEFT JOIN `" . jsjobs::$_db->prefix . "js_job_categories` AS category ON category.id = resume.job_category
        LEFT JOIN `" . jsjobs::$_db->prefix . "js_job_experiences` AS exp ON exp.id = resume.experienceid
        ...
        LEFT JOIN `" . jsjobs::$_db->prefix . "js_job_states` AS state ON state.id = city.stateid
        LEFT JOIN `" . jsjobs::$_db->prefix . "js_job_countries` AS country ON country.id = city.countryid";
    $where = ' WHERE ' . $whereClauseCore;
    $order = " ORDER BY " . jsjobs::$_ordering;
    $limit = " LIMIT " . JSJOBSpagination::$_offset . "," . JSJOBSpagination::$_limit;

        $query = $this->prepareSql($select . $from . $where . $order . $limit, $filterParams);
    jsjobs::$_data['debug_myresumes_sql']['listQuery'] = $query;
        $results = jsjobs::$_db->get_results($query);
        $listQueryError = jsjobs::$_db->last_error;
        $data = array();
        $resultCount = is_array($results) ? count($results) : 0;
        $statuses = array();
        if (is_array($results)) {
            foreach ($results as $row) {
                $statuses[] = isset($row->status) ? (int) $row->status : null;
            }
        }
        $sampleWhereClauses = array();
        $sampleParams = array();
        $sampleWhere = $whereClauseCore;
        $sampleParams = $filterParams;
        $sampleRows = array();
        $sampleQuery = null;
        if (!empty($sampleWhere)) {
            $sampleQuery = $this->prepareSql(
                "SELECT id, uid, status, application_title FROM `" . jsjobs::$_db->prefix . "js_job_resume` WHERE $sampleWhere ORDER BY id DESC LIMIT 5",
                $sampleParams
            );
            $sampleRows = jsjobs::$_db->get_results($sampleQuery, ARRAY_A);
        }
        if ($sampleQuery !== null) {
            jsjobs::$_data['debug_myresumes_sql']['sampleQuery'] = $sampleQuery;
        }
    $usedEmailFallback = ($emailOnlyCount !== null && $emailOnlyCount > 0) || ($autoPatchedCount > 0);
        $lastResolvedUid = get_option('jsjobs_debug_last_resolved_uid');
        $lastUidFix = get_option('jsjobs_debug_last_uid_fix');
        $uidInsertError = get_option('jsjobs_debug_uid_insert_error');
        $userTableRow = jsjobs::$_db->get_row(
            $this->prepareSql(
                "SELECT id, uid, emailaddress, first_name, last_name FROM `" . jsjobs::$_db->prefix . "js_job_users` WHERE id = %d",
                array($uid)
            ),
            ARRAY_A
        );
        ...
        $legacyUserRow = null;
        if (!empty($generalDiagnostics)) {
            foreach ($generalDiagnostics as $diagRow) {
                if (isset($diagRow['email_address'])) {
                    $legacyUserRow = jsjobs::$_db->get_row(
                        $this->prepareSql(
                            "SELECT id, uid, emailaddress, first_name, last_name FROM `" . jsjobs::$_db->prefix . "js_job_users` WHERE emailaddress = %s",
                            array($diagRow['email_address'])
                        ),
                        ARRAY_A
                    );
        ...
        if ($autoPatchedCount > 0) {
            jsjobs::$_data['debug_myresumes_uid_patched'] = $autoPatchedCount;
            jsjobs::$_data['debug_myresumes_uid_patched_ids'] = $autoPatchedIds;
        }
        ...
        jsjobs::$_data['debug_myresumes'] = array(
            ...
            'limit' => (int) JSJOBSpagination::$_limit,
            'sample' => $sampleRows,
            'emailFallbackTotal' => $emailOnlyCount,
            'uidFallbackTotal' => $uidOnlyCount,
            'forcedUid' => isset(jsjobs::$_data['debug_myresumes_forced_uid']) ? (int) jsjobs::$_data['debug_myresumes_forced_uid'] : null,
            'usedEmailFallback' => $usedEmailFallback,
            'email' => !empty($currentUserEmail) ? $currentUserEmail : null,
            'lastResolvedUid' => $lastResolvedUid !== false ? (int) $lastResolvedUid : null,
            'lastUidFix' => $lastUidFix !== false ? (int) $lastUidFix : null,
            'uidInsertError' => $uidInsertError !== false ? $uidInsertError : null,
            'listQueryError' => !empty($listQueryError) ? $listQueryError : null,
            'fallbackTriggered' => ($total === 0),
            'fallbackTriggeredInitial' => $fallbackTriggeredInitial,
            'emailDiagnostics' => $rawEmailDiagnostics,
            'uidDiagnostics' => $rawUidDiagnostics,
            'generalDiagnostics' => $generalDiagnostics,
            'userRecord' => $userTableRow,
            'legacyUserRecord' => $legacyUserRow,
            'trackedResumeIds' => $trackedResumeIds,
            'trackedResumeEmails' => $trackedResumeEmails,
        );
        foreach ($results AS $d) {
            $d->salary = JSJOBSincluder::getJSModel('common')->getSalaryRangeView($d->symbol, $d->rangestart, $d->rangeend, $d->rangetype);
            $d->location = JSJOBSincluder::getJSModel('common')->getLocationForView($d->cityname, $d->statename, $d->countryname);
            if ($wpUserId > 0 && isset($d->id)) {
                $this->trackResumeOwnership($wpUserId, (int) $d->id, isset($d->email_address) ? $d->email_address : '');
            }
            $data[] = $d;
        }
        jsjobs::$_data['fields'] = JSJOBSincluder::getJSModel('fieldordering')->getFieldsOrderingforView(3);
...
//        ob_clean();
    ...
    function getIfResumeOwner($jobid) {
        ...
        $userObject = JSJOBSincluder::getObjectClass('user');
        $uid = $userObject->uid();
        $resolvedUid = is_numeric($uid) ? (int) $uid : 0;
        $emailAddress = $userObject->emailaddress();
        $wpUserId = is_user_logged_in() ? get_current_user_id() : 0;

        if (($resolvedUid <= 0) && $wpUserId > 0) {
            $fallbackUid = $userObject->getjsjobsuidbyuserid($wpUserId);
            if (!is_numeric($fallbackUid) || (int) $fallbackUid <= 0) {
                $fallbackUid = $this->ensureJsJobsUserForCurrentWpUser($wpUserId);
            }
            if (is_numeric($fallbackUid) && (int) $fallbackUid > 0) {
                $resolvedUid = (int) $fallbackUid;
                $userObject = JSJOBSincluder::getObjectClass('user');
            }
        }

        $resumeRow = jsjobs::$_db->get_row(
            jsjobs::$_db->prepare(
                "SELECT id, uid, email_address FROM `" . jsjobs::$_db->prefix . "js_job_resume` WHERE id = %d",
                (int) $jobid
            )
        );

        $ownerDiagnostics = array(
            'jobid' => (int) $jobid,
            'resolvedUid' => $resolvedUid,
            'email' => $emailAddress,
            'resumeRow' => $resumeRow ? (array) $resumeRow : null,
        );

        if ($resumeRow) {
            $resumeEmail = isset($resumeRow->email_address) ? $resumeRow->email_address : '';
            $resumeUid = isset($resumeRow->uid) ? (int) $resumeRow->uid : 0;
            $emailUpdated = false;

            if (!empty($emailAddress) && empty($resumeEmail)) {
                $updateData = array('email_address' => $emailAddress);
                $updateFormat = array('%s');
                if ($resolvedUid > 0) {
                    $updateData['uid'] = $resolvedUid;
                    $updateFormat[] = '%d';
                }
                jsjobs::$_db->update(
                    jsjobs::$_db->prefix . 'js_job_resume',
                ...
                if (jsjobs::$_db->rows_affected > 0) {
                    $resumeEmail = $emailAddress;
                    if ($resolvedUid > 0) {
                        $resumeUid = $resolvedUid;
                        update_option('jsjobs_debug_last_uid_fix', $resolvedUid);
                        $ownerDiagnostics['uidUpdated'] = $resolvedUid;
                    }
                    $emailUpdated = true;
                    if ($wpUserId > 0) {
                        $this->trackResumeOwnership($wpUserId, (int) $jobid, $resumeEmail);
                    }
                }
            }

            if ($resolvedUid > 0 && $resumeUid === 0) {
                jsjobs::$_db->update(
                    jsjobs::$_db->prefix . 'js_job_resume',
                    array('uid' => $resolvedUid),
                    array('id' => (int) $jobid),
            ...
            if ($emailUpdated) {
                $ownerDiagnostics['emailUpdated'] = $resumeEmail;
            }

            $ownerDiagnostics['resumeRow'] = array(
                'id' => isset($resumeRow->id) ? $resumeRow->id : (int) $jobid,
                'uid' => $resumeUid,
                'email_address' => $resumeEmail,
            );

            if (!empty($resumeEmail) && !empty($emailAddress) && strcasecmp($resumeEmail, $emailAddress) === 0) {
                $ownerDiagnostics['matchedBy'] = 'email';
                update_option('jsjobs_debug_owner_check', $ownerDiagnostics);
                return true;
            }
            if ($resolvedUid > 0 && $resumeUid === $resolvedUid) {
                $ownerDiagnostics['matchedBy'] = 'uid';
                update_option('jsjobs_debug_owner_check', $ownerDiagnostics);
                return true;
            }
        }

        if (!empty($emailAddress) && $wpUserId > 0) {
            $ownerDiagnostics['matchedBy'] = 'none';
            update_option('jsjobs_debug_owner_check', $ownerDiagnostics);
        }
