...
<?php
    ...
    $viewDebug = array();
    if (isset(jsjobs::$_data['debug_resume']) && is_array(jsjobs::$_data['debug_resume'])) {
        $viewDebug['controller'] = jsjobs::$_data['debug_resume'];
    }
    $ownerCheckOption = get_option('jsjobs_debug_owner_check');
    if ($ownerCheckOption !== false) {
        $viewDebug['ownerCheck'] = $ownerCheckOption;
    }
    $lastUidFix = get_option('jsjobs_debug_last_uid_fix');
    if ($lastUidFix !== false) {
        $viewDebug['lastUidFix'] = (int) $lastUidFix;
    }
    $lastResolvedUid = get_option('jsjobs_debug_last_resolved_uid');
    if ($lastResolvedUid !== false) {
        $viewDebug['lastResolvedUid'] = (int) $lastResolvedUid;
    }
    $uidInsertError = get_option('jsjobs_debug_uid_insert_error');
    if ($uidInsertError !== false) {
        $viewDebug['uidInsertError'] = $uidInsertError;
    }
    if (!empty($viewDebug)) {
        $viewDebugJson = wp_json_encode($viewDebug);
        if ($viewDebugJson !== false) {
            echo '<script>console.group("[JS Jobs] View Resume diagnostics");console.log(' . $viewDebugJson . ');console.groupEnd();</script>';
        }
    }
