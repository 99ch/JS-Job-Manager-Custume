...
if (jsjobs::$_error_flag == null) {
    ...
    <div id="jsjobs-wrapper">
        <?php
        $debugPayload = array();
        if (isset(jsjobs::$_data['debug_myresumes']) && is_array(jsjobs::$_data['debug_myresumes'])) {
            $debugPayload['core'] = jsjobs::$_data['debug_myresumes'];
        }
        if (isset(jsjobs::$_data['debug_myresumes_sql'])) {
            $debugPayload['sql'] = jsjobs::$_data['debug_myresumes_sql'];
        }
        if (isset(jsjobs::$_data['debug_myresumes_uid_patched'])) {
            $debugPayload['uidPatchedCount'] = (int) jsjobs::$_data['debug_myresumes_uid_patched'];
        }
        $lastOwnerCheck = get_option('jsjobs_debug_owner_check');
        if ($lastOwnerCheck !== false) {
            $debugPayload['lastOwnerCheck'] = $lastOwnerCheck;
        }
        $lastUidFix = get_option('jsjobs_debug_last_uid_fix');
        if ($lastUidFix !== false) {
            $debugPayload['lastUidFix'] = (int) $lastUidFix;
        }
        $lastResolvedUid = get_option('jsjobs_debug_last_resolved_uid');
        if ($lastResolvedUid !== false) {
            $debugPayload['lastResolvedUid'] = (int) $lastResolvedUid;
        }
        $uidInsertError = get_option('jsjobs_debug_uid_insert_error');
        if ($uidInsertError !== false) {
            $debugPayload['uidInsertError'] = $uidInsertError;
        }
        if (!empty($debugPayload)) {
            $debugJson = wp_json_encode($debugPayload);
            if ($debugJson !== false) {
                echo '<script>console.group("[JS Jobs] My Resumes diagnostics");console.log(' . $debugJson . ');console.groupEnd();</script>';
            }
        }
        ?>
