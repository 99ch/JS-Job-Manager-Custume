...
class JSJOBScurrencyModel {
    ...
    function getAllCurrencies() {
        ...
        if ($code != null)
            $inquery .=$clause . " code LIKE '%" . esc_sql($code) . "%'";
    ...
    function isCurrencyExist($title) {
        $query = "SELECT COUNT(id) FROM " . jsjobs::$_db->prefix . "js_job_currencies WHERE title = '" . esc_sql($title) . "'";
    ...
    function currencyCanUnpulish($currencyid) {
        $query = " SELECT COUNT(id) FROM `" . jsjobs::$_db->prefix . "js_job_currencies` AS cur WHERE cur.id = " . esc_sql($currencyid) . " AND cur.default = 1 ";
        $total = jsjobsdb::get_var($query);
        if ($total > 0)
            return false;
        else
