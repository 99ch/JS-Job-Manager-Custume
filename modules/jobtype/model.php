...
class JSJOBSJobtypeModel {
    ...
    function isJobTypesExist($title) {
        if (!$title)
            return false;
        $query = "SELECT COUNT(id) FROM " . jsjobs::$_db->prefix . "js_job_jobtypes WHERE title = '" . esc_sql($title) . "'";
    ...
    function getIDByTitle($title) {
        ...
        $query = "SELECT id FROM " . jsjobs::$_db->prefix . "js_job_jobtypes WHERE title = '" . esc_sql($title) ."'";
        $title = jsjobsdb::get_var($query);
        return $title;
    }
