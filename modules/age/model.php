...
class JSJOBSageModel {
    ...
    function getJobAgesbyId($id) {
        ...
        $query = "SELECT * FROM " . jsjobs::$_db->prefix . "js_job_ages WHERE id = " . esc_sql($id);
    ...
    function getAllAges() {
        ...
        if (is_numeric($status))
            $inquery .=$clause . " status = " . esc_sql($status);
    ...
    function updateIsDefault($id) {
        ...
        if (is_numeric($id)) {
            $query = "UPDATE `" . jsjobs::$_db->prefix . "js_job_ages` SET isdefault = 0 WHERE id != " . esc_sql($id);
    ...
    function ageCanUnpublish($ageid) {
        if (!is_numeric($ageid))
            return false;
        $query = " SELECT
                    ( SELECT COUNT(id) FROM `" . jsjobs::$_db->prefix . "js_job_ages` WHERE id = " . esc_sql($ageid) . " AND isdefault = 1)
                    AS total";
        $total = jsjobsdb::get_var($query);
        if ($total > 0)
            return false;
    ...
    function ageCanDelete($ageid) {
        if (!is_numeric($ageid))
            return false;
        $query = " SELECT
                    ( SELECT COUNT(id) FROM `" . jsjobs::$_db->prefix . "js_job_jobs` WHERE agefrom = " . esc_sql($ageid) . " OR ageto = " . esc_sql($ageid) . ")
                    + ( SELECT COUNT(id) FROM `" . jsjobs::$_db->prefix . "js_job_ages` WHERE id = " . esc_sql($ageid) . " AND isdefault = 1)
                    AS total";
        $total = jsjobsdb::get_var($query);
        if ($total > 0)
