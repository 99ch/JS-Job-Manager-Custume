    ...
    function fieldsRequiredOrNot($ids, $value) {
        ...
        foreach ($ids as $id) {
            if(is_numeric($id) && is_numeric($value)){
                $query = "UPDATE " . jsjobs::$_db->prefix . "js_job_fieldsordering SET required = " . $value . " WHERE id = " . esc_sql($id) . " AND sys=0";
...
//had make changes impliment fieldtitle in view compnay
    ...
    function fieldsPublishedOrNot($ids, $value) {
        ...
        foreach ($ids as $id) {
            if(is_numeric($id) && is_numeric($value)){
                $query = "UPDATE " . jsjobs::$_db->prefix . "js_job_fieldsordering SET published = " . $value . " WHERE id = " . esc_sql($id) . " AND cannotunpublish=0";
                if (false === jsjobsdb::query($query)) {
                    $total += 1;
                }
            }else{
    ...
    function visitorFieldsPublishedOrNot($ids, $value) {
        ...
        foreach ($ids as $id) {
            if(is_numeric($id) && is_numeric($value)){
                $query = "UPDATE " . jsjobs::$_db->prefix . "js_job_fieldsordering SET isvisitorpublished = " . $value . " WHERE id = " . esc_sql($id) . " AND cannotunpublish=0";
                if (false === jsjobsdb::query($query)) {
                    $total += 1;
                }
            }else{
    ...
    function storeUserField($data) {
        ...
        if ($data['isuserfield'] == 1) {
            ...
            if ($data['id'] == '') { // only for new
                $query = "SELECT max(ordering) FROM " . jsjobs::$_db->prefix . "js_job_fieldsordering WHERE fieldfor = " . esc_sql($data['fieldfor']);
    ...
    function getFieldsForComboByFieldFor() {
        ...
        $editquery = '';
        if($parentfield){
            $query = "SELECT id FROM " . jsjobs::$_db->prefix . "js_job_fieldsordering WHERE fieldfor = $fieldfor AND (userfieldtype = 'radio' OR userfieldtype = 'combo' OR userfieldtype = 'depandant_field') AND depandant_field = '" . esc_sql($parentfield) . "' ";
    ...
    function getSectionToFillValues() {
        ...
        $datas = array();
        if(is_numeric($field)){
            $query = "SELECT userfieldparams FROM " . jsjobs::$_db->prefix . "js_job_fieldsordering WHERE id=".esc_sql($field);
    ...
    function getUserfieldsfor($fieldfor, $resumesection = null) {
        ...
        $query = "SELECT field,userfieldparams,userfieldtype FROM `" . jsjobs::$_db->prefix . "js_job_fieldsordering` WHERE fieldfor = " . $fieldfor . " AND isuserfield = 1 AND " . esc_sql($published);
    ...
    function makeDependentComboFiledForResume($val,$childfield,$type,$section,$sectionid,$themecall){
        ...
        $query = "SELECT field,depandant_field,userfieldparams,fieldtitle, required FROM `".jsjobs::$_db->prefix."js_job_fieldsordering` WHERE field = '".esc_sql($childfield)."'";
    ...
    function getFieldsForListing($fieldfor){
        ...
        $query = "SELECT field  FROM " . jsjobs::$_db->prefix . "js_job_fieldsordering WHERE showonlisting = 1 AND " . esc_sql($published) . " AND fieldfor =" . esc_sql($fieldfor) ;
        $data = jsjobsdb::get_results($query);
        $return_array = array();
