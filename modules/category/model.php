...
class JSJOBSCategoryModel {
    ...
    function getCategorybyId($id,$count_flag = 0) {
        ...
        $query = " SELECT * FROM " . jsjobs::$_db->prefix . "js_job_categories WHERE id = " . esc_sql($id);
    ...
    function updateIsDefault($id) {
        ...
        $query = "UPDATE `" . jsjobs::$_db->prefix . "js_job_categories` SET isdefault = 0 WHERE id != " . esc_sql($id);
    ...
    function validateFormData(&$data) {
        ...
        if ($data['id'] == '') {
            ...
            if ($result == true) {
                ...
                $query = "SELECT max(ordering)+1 AS maxordering FROM " . jsjobs::$_db->prefix . "js_job_categories " . esc_sql($inquery);
    ...
    function categoryCanUnpublish($categoryid) {
        if (!is_numeric($categoryid))
            return false;
        $query = "SELECT
                    ( SELECT COUNT(id) FROM `" . jsjobs::$_db->prefix . "js_job_categories` WHERE id = " . esc_sql($categoryid) . " AND isdefault = 1)
                    AS total ";
        $total = jsjobsdb::get_var($query);
        if ($total > 0)
            return false;
    ...
    function categoryCanDelete($categoryid) {
        if (!is_numeric($categoryid))
            return false;
        $query = "SELECT
                    ( SELECT COUNT(id) FROM `" . jsjobs::$_db->prefix . "js_job_companies` WHERE category = " . esc_sql($categoryid) . ")
                    +( SELECT COUNT(id) FROM `" . jsjobs::$_db->prefix . "js_job_jobs` WHERE jobcategory = " . esc_sql($categoryid) . ")
                    +( SELECT COUNT(id) FROM `" . jsjobs::$_db->prefix . "js_job_resume` WHERE job_category = " . esc_sql($categoryid) . ")
                    +( SELECT COUNT(id) FROM `" . jsjobs::$_db->prefix . "js_job_categories` WHERE id = " . esc_sql($categoryid) . " AND isdefault = 1)
                    AS total ";
        $total = jsjobsdb::get_var($query);
        if ($total > 0)
    ...
    function isCategoryExist($title) {
        ...
        $query = "SELECT COUNT(id) FROM " . jsjobs::$_db->prefix . "js_job_categories WHERE cat_title = '" . esc_sql($title) . "'";
