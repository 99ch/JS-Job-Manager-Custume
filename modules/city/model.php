...
class JSJOBSCityModel {
    ...
    function getAllStatesCities($countryid, $stateid) {
        ...
        if ($countryid) {
            $inquery .= $clause . "countryid = " . esc_sql($countryid);
            $clause = ' AND ';
        }
    ...
    function isCityExist($countryid, $stateid, $title) {
		...
		AND stateid=" . $stateid . " AND LOWER(name) = '" . esc_sql(jsjobslib::jsjobs_strtolower($title)) . "'";
    ...
    function getAddressDataByCityName($cityname, $id = 0) {
        ...
        if ($id == 0) {
            if (isset($countryname)) {
                $query .= " WHERE city.name LIKE '" . esc_sql($cityname) . "%' AND country.name LIKE '" . esc_sql($countryname) . "%' AND country.enabled = 1 AND city.enabled = 1 AND IF(state.name is not null,state.enabled,1) = 1 LIMIT " . JSJOBSincluder::getJSModel('configuration')->getConfigValue("number_of_cities_for_autocomplete");
                ...
                $query .= " WHERE city.name LIKE '" . esc_sql($cityname) . "%' AND country.enabled = 1 AND city.enabled = 1 AND IF(state.name is not null,state.enabled,1) = 1 LIMIT " . JSJOBSincluder::getJSModel('configuration')->getConfigValue("number_of_cities_for_autocomplete");
